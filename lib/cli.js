...
program
  ...
  .option('stop   <appname>', 'Stop the application')
  .option('purge  <appname>', 'Delete the application')
  .parse(process.argv);
...
function streamHandler(err, stream) {
  ...
  stream.on('data', function(data) {
    console.log(JSON.parse(data).stream);
  });
}
...
function createTableApps(apps) {
  ...
  apps.forEach(function(app) {
    table.push([app.id.substring(0,12), app.name]);
  });
